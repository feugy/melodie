...
import { getLogger, hash } from '../../utils/index.js'
import * as tag from './tag-reader.js'
 ...
 */
export function isPlaylistFile(path) {
  ...
  try {
    ...
    for (const line of lines.split('\n')) {
      if (
        ...
        let path = line.startsWith('file://') ? decodeURI(line).slice(7) : line
        if (!isAbsolute(path)) {
 ...
 */
export async function write(path, playlist) {
