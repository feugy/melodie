...
describe('Cover service', () => {
	...
	describe('findInFolder()', () => {
		...
		it('returns undefined for cover-less path', async () => {
			expect(await service.findInFolder(join(path, 'file.mp3'))).toBeUndefined()
		})
	...
	describe('findAlbumCover()', () => {
		...
		it('returns all images inside folders', async () => {
			...
			vi.mocked(tracksModel).getById.mockResolvedValueOnce({
				id: hash(files[0]),
				mtimeMs: Date.now(),
				path: files[0],
